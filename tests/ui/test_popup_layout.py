...
def test_popup_layout_scales_logical_defaults_and_stays_in_work_area(scale: float) -> None:
    ...
    assert bounds.height == min(336, int(1080 / scale * 0.70))
...
def test_popup_layout_keeps_stable_height_for_short_and_long_content() -> None:
    ...
    assert policy.calculate(metrics, content_lines=4).height == 336
    assert policy.calculate(metrics, content_lines=10).height == 336
    assert policy.calculate(metrics, content_lines=30).height == 336
...
def test_comfortable_physical_size_matches_reference_displays() -> None:
    ...
    assert (round(desktop.width * 1.25), round(desktop.height * 1.25)) == (500, 420)
    assert (round(laptop.width * 2.0), round(laptop.height * 2.0)) == (800, 672)
