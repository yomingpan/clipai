...
def test_synthetic_catalog_to_extracted_bundle_and_request(tmp_path: Path):
    ...
    request = UpdateRequestArtifact(transaction_id("synthetic-transaction"), "2026-09-13T00:00:00+00:00", "3.7.3", parsed_manifest.app_version, (tmp_path / "old-python.exe").resolve(), 1234, bundle.resolve(), bundle.stat().st_size, built.bundle_sha256, built.manifest_sha256, "synthetic-key", (tmp_path / "install").resolve(), (tmp_path / "shared").resolve(), "synthetic-install")
