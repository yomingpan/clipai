...
6. For every text-capable Action, prefer text explicitly selected at the instant the user triggers the Action. Automatically fall back to the clipboard only after a source-bound probe confirms no selection. Unsupported providers, timeouts, changed sources, and cancellation must never be treated as no selection; an explicit clipboard intent may use frozen clipboard input. Selection capture must not permanently change the user's previous clipboard content. See `docs/contracts/services/selection-capture-contract.md`.
7. `ProviderExecutionModule` exclusively owns async provider HTTP tasks, transport-level cancellation and settlement, connection pooling, and transport shutdown. `WorkflowController` decides whether its active invocation is cancelled; it does not own or manipulate the transport task directly.
