...
4. Run `python scripts/validate_language_packs.py`; verify every official pack, review record, fixed-output-language contract, and clean-checkout checksum.
5. On a clean Windows checkout, run `run_clipai.bat` and verify startup. If Action Language resources changed, exercise Tray selection, restart-only activation, selected-pack recovery, and default-pack fail-closed behavior.
