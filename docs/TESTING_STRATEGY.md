...
- Voice Draft 編輯測試必須覆蓋重疊的 physical KeyPress／KeyRelease：較新字元已由
  widget 插入、但尚未送達 Workflow 時，較舊 edit acknowledgment 不得重建全文、
  清除 selection 或移動 live caret。Widget mutation 應恰好送出一次 canonical edit，
  程式化 projection 不得回送成新的使用者 edit。
