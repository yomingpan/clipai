...
Hotkey callback 必須先 enqueue atomic `ShortcutTriggered(shortcut_id, press_type)`；
command resolution 在 app command pump 內由 `ShortcutSequenceCoordinator`
執行。Coordinator 擁有 sequence waiting、timeout 與 cancellation policy；
`ShortcutCatalog` 保持 atomic trigger 到 typed command 的 mapping，platform
listener 不保存 sequence state。
