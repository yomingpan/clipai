...
Application startup waits only for the provider event loop to accept work. Shared transport initialization runs asynchronously on that loop; provider operations await the same initialization task before making a request. Cancelling one operation does not cancel the shared initialization. A failed initialization is reported through the operation error channel and may be retried by a later explicit operation instead of preventing the application from starting.
...
- Slow or failed transport initialization cannot make application startup fail solely because a fixed readiness deadline elapsed.
