...
_CUSTOMTKINTER_ICON_DELAY_MS = 250


def _install_windows_window_icons(window) -> tuple[int, ...]:
    """Set the title-bar and taskbar icon after CustomTkinter's default icon."""
    ...
    if not window_handle:
        raise OSError("Unable to resolve the native Provider Settings window")
...
class ProviderSettingsDialog:
    ...
    def __init__(self, master, command_sink: Callable[[object], None]) -> None:
        ...
        self._window.after(_CUSTOMTKINTER_ICON_DELAY_MS, self._apply_windows_window_icons)
    ...
    def _apply_windows_window_icons(self) -> None:
        try:
            self._window_icon_handles = _install_windows_window_icons(self._window)
    ...
    def destroy(self) -> None:
        try:
            ...
            pass
        if sys.platform == "win32":
            for icon_handle in self._window_icon_handles:
